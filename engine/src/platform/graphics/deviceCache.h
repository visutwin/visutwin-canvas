...
namespace visutwin::canvas
{
    ...
    {
    public:
        void remove(GraphicsDevice* device);
    ...
    std::shared_ptr<T> DeviceCache::get(const std::shared_ptr<GraphicsDevice>& device, std::function<std::shared_ptr<T>()> onCreate) {
        ...
        if (it == _cache.end()) {
            ...
            // Set up event handlers for the device lifecycle. Capture a raw
            // pointer, NOT the shared_ptr: these callbacks live on the device's
            // own EventHandler, so a strong capture would be a self-cycle that
            // keeps the device (and every GPU resource it owns) alive forever.
            // The raw pointer stays valid — the callbacks die with the device.
            auto* deviceRaw = device.get();
            auto destroyCallback = [this, deviceRaw]() {
                remove(deviceRaw);
            };

            auto deviceLostCallback = [this, deviceRaw]() {
                handleDeviceLost(deviceRaw);
            };
