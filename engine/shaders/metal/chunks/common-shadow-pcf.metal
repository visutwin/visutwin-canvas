...
// depth2d_array variant for the clustered spot-shadow atlas: same optimized 4-tap
// PCF, but sampling array layer `slice`.
static inline float getShadowPCF3x3Array(depth2d_array<float> shadowMap, float2 shadowUv,
                                         float depth, float resolution, uint slice) {
    constexpr sampler shadowCompSampler(coord::normalized, filter::linear,
                                        compare_func::less_equal, address::clamp_to_edge);
    ...
    float sum = 0.0;
    sum += uw0 * vw0 * shadowMap.sample_compare(shadowCompSampler, float2(u0, v0), slice, z);
    sum += uw1 * vw0 * shadowMap.sample_compare(shadowCompSampler, float2(u1, v0), slice, z);
    sum += uw0 * vw1 * shadowMap.sample_compare(shadowCompSampler, float2(u0, v1), slice, z);
    sum += uw1 * vw1 * shadowMap.sample_compare(shadowCompSampler, float2(u1, v1), slice, z);
