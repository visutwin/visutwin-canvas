...
// Many colored local lights (omni + spot, no per-light shadows) light a field of
// objects. With Scene::setClusteredLightingEnabled(true) the renderer buckets the
// lights into a 3D world-space grid (WorldClusters) so each fragment only evaluates
// the lights in its cell — cheap even with dozens of lights.
//
...
int main()
{
    ...
    // Enable clustered lighting: local lights are bucketed into a 3D grid so many
    // of them can be evaluated cheaply. Local-light shadows are not used here.
    ...
    spdlog::info("{} local lights (clustered): 24 omni + 8 spot, no per-light shadows.", lights.size());
